 ...
 */
package com.example.virtualtourar.samplerender;
...
public class Texture implements Closeable {
   ...
   * {@link #createFromAsset} or {@link #createFromBitmap} if you want a texture with data.
   */
  ...
  /** Create a texture directly from a {@link Bitmap}. (No mipmaps; good for dynamic UI cards.) */
  public static Texture createFromBitmap(
          SampleRender render, Bitmap bitmap, WrapMode wrapMode, ColorFormat colorFormat) {
    Texture texture = new Texture(render, Target.TEXTURE_2D, wrapMode, /*useMipmaps=*/ false);
    texture.updateFromBitmap(bitmap, colorFormat);
    return texture;
  }

  /** Update this texture's contents from a {@link Bitmap}. (No mipmaps generated.) */
  public void updateFromBitmap(Bitmap bitmap, ColorFormat colorFormat) {
    // Ensure RGBA_8888 backing
    Bitmap rgba = convertBitmapToConfig(bitmap, Bitmap.Config.ARGB_8888);
    ByteBuffer buffer = ByteBuffer.allocateDirect(rgba.getByteCount());
    ...
    GLES30.glTexImage2D(
            ...
            GLES30.GL_UNSIGNED_BYTE,
            buffer);
    GLError.maybeThrowGLException("Failed to upload texture data", "glTexImage2D");
    // No mipmaps for dynamic content
    if (rgba != bitmap) {
      rgba.recycle();
    }
  }
  ...
  /* package-private */ Target getTarget() {
    return target;
  }
